...
#ifndef _distcc_common_trace_h_
#define _distcc_common_trace_h_
...
rs_loglevel rs_loglevel_from_name(const char *name);

enum
{
    RS_LOG_PRIMASK      =  7,   // Mask to extract priority part
    RS_LOG_NONAME       =  8,   // Don't show function name in message
 ...
 */
typedef void rs_logger_fn(int flags, const char *file, int line, char const *msg, va_list,
...
void rs_logger_file(int level, const char *file, int line, char const *fmt, va_list va, void *, int);
void rs_logger_syslog(int level, const char *file, int line, char const *fmt, va_list va, void *, int);
...
#define rs_trace(fmt, ...) \
    do \
		rs_log0(RS_LOG_DEBUG, __FILE__, __LINE__, fmt ,##__VA_ARGS__);  \
	while (0)
#else
#define rs_trace(s, ...)
#endif	// !DO_RS_TRACE
...
#define rs_log(l, s, ...) \
	do \
		rs_log0((l), __FILE__, __LINE__, (s) ,##__VA_ARGS__);  \
	while (0)

#define rs_log_crit(s, ...) \
	do \
		rs_log0(RS_LOG_CRIT, __FILE__, __LINE__, (s) ,##__VA_ARGS__); \
	while (0)

#define rs_log_error(s, ...) \
	do \
		rs_log0(RS_LOG_ERR, __FILE__, __LINE__, (s) ,##__VA_ARGS__); \
	while (0)

#define rs_log_notice(s, ...) \
	do \
		rs_log0(RS_LOG_NOTICE, __FILE__, __LINE__, (s) ,##__VA_ARGS__); \
	while (0)

#define rs_log_warning(s, ...) \
	do \
		rs_log0(RS_LOG_WARNING, __FILE__, __LINE__, (s) ,##__VA_ARGS__); \
	while (0)

#define rs_log_info(s, ...) \
	do \
		rs_log0(RS_LOG_INFO, __FILE__, __LINE__, (s) ,##__VA_ARGS__); \
	while (0)

#else // not defined HAVE_VARARG_MACROS

// If we don't have gcc vararg macros, then we fall back to making the log routines just plain functions.
// On platforms without gcc (boo hiss!) this means at least you get some messages, but not the nice
// function names etc.
...
extern int __thread log_context;

extern rvfc::AtomicUINT32 serial_log_context;

#endif // _distcc_common_trace_h_
