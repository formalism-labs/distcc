...
void s$get_module_info (char_varying *module_name, void *mip, short int *code);
...
#warning "Please port this function"
    ...
    if (ncpus == -1) 
	{
        rs_log_error("sysconf(_SC_NPROCESSORS_ONLN) failed: %s", strerror(errno));
        ncpus = 1;
        return EXIT_DISTCC_FAILED;
    }
	else if (ncpus == 0) 
	{
		...
		// But it has apparently been observed to happen on ARM Linux
		ncpus = 1;
    }
