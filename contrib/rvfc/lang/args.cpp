...
Arguments Arguments::pop_front(int n)
{
	if (n > count())
		n = count();
	std::list<text> args;
...
Arguments Arguments::pop_back(int n)
{
	if (n > count())
		n = count();
	std::list<text> args;
...
{
	int n = _args.size() + 1; // +1 for extra null pointer at and of args list
	int size_in_chars = sizeof(char*) * n;
	for (ConstIterator i = *this; !!i; ++i)
