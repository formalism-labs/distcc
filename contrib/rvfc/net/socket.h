...
#ifndef _rvfc_net_socket_
#define _rvfc_net_socket_

#include "rvfc/net/classes.h"
#include "rvfc/net/address.h"
#include "rvfc/net/host.h"
...
public:
	CLASS_EXCEPTION_DEF("rvfc::SocketError");
...
protected:
	...
	void _invalidate();
	void _invalidate() const { rvfc::_invalidate(); }
...
public:
	...
	//-----------------------------------------------------------------------------------------
public:
	CLASS_EXCEPTION_DEF("rvfc::Socket");
...
public:
	...
	//-----------------------------------------------------------------------------------------
public:
	CLASS_EXCEPTION_DEF("rvfc::TcpSocket");
	CLASS_SUB_EXCEPTION_DEF(InitErr, "initialization error"); 
...
public:
	TcpServerSocket(const Port &port, int backlog = 128) : Socket(Uninitialized()) { ctor(LocalSocketAddress(port), backlog); }
	TcpServerSocket(const LocalSocketAddress &host, int backlog = 128) : Socket(Uninitialized()) { ctor(host, backlog); }
	...
	CLASS_EXCEPTION_DEF("rvfc::TcpServerSocket");
	CLASS_SUB_EXCEPTION_DEF(BindErr, "local address is already in use - bind problem");
};
...
public:
	...
	CLASS_EXCEPTION_DEF("rvfc::UdpPeerSocket");
