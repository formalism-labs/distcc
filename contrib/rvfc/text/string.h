...
#ifndef _rvfc_types_string_
#define _rvfc_types_string_
...
#ifdef RVFC_TARGET_OS_VXWORKS_5
#define vsnprintf(buffer, count, format, args) vsprintf((buffer), (format), (args)) 
#endif
...
inline char *alloc_va_args_buffer(const char *fmt, char *buf, size_t buf_size, va_list args)
{
	...
	for (;;)
	{
		...
		if (chars >= 0 && (size_t) chars < size)
			return buf;
		size *= 2;
		p = (char *) (size == 2 * sizeof(buf) ? malloc(size) : realloc(p, size));
