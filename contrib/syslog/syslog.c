...
#else
#define SYSLOG_DGRAM_SIZE 10240
#endif
...
// thread-specific
static __declspec(thread) char datagramm[SYSLOG_DGRAM_SIZE];
...
void openlog( char* ident, int option, int facility )
{
    ...
    if( option & LOG_PID )
        _snprintf(str_pid, sizeof(str_pid), "[%lu %lx]", (int) GetCurrentProcessId(), (int) GetCurrentThreadId());
    else
        str_pid[0] = 0;
